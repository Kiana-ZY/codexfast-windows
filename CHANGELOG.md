...
- Made reconnect verification use a true 15-second wall-clock deadline, reload confirmed existing renderers before lazy-resource preload, ignore stale Fetch callbacks from older connection generations, and cancel/drain pending CDP connections and handlers before launcher exit.
- Accepted empty-URL renderers that are not paused by querying their current location without mutation, reloading only after an `app://` origin is confirmed, allowing a later valid app renderer to complete startup, and triggering preload even when binding arrives after the initial wait window.
- Isolated the Windows tray mutex/start events by project path, added acknowledged second-invocation signaling, pinned the shortcut to its validated Node executable, removed `cmd.exe` path expansion from launcher logging, and replaced shortcuts atomically.
